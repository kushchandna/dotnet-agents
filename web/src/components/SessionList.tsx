  ...
  return (
    <div className="session-list" data-testid="session-list">
      <div className="session-list-header">
        <span>Sessions</span>
        <button onClick={onNew} aria-label="New session">+ New</button>
      </div>
      <ul>
        {sessions.map((s) => (
          <li key={s.id} className={s.id === activeSessionId ? 'active' : ''} data-testid={`session-item-${s.id}`}>
            <button className="session-select" onClick={() => onSelect(s.id)}>
              {s.title || new Date(s.createdAt).toLocaleString()}
            </button>
            <button className="session-delete" aria-label={`Delete session ${s.id}`} onClick={() => onDelete(s.id)}>×</button>
          </li>
        ))}
      </ul>
    </div>
